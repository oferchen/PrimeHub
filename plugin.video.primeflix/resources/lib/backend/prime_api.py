...
class _JsonRPCIntegration:
    ...
    def get_playable(self, asin: str) -> Playable:
        ...
        data = self._execute_action(
            "get_playable", asin=asin, region=region_str, resolution=resolution_str
        )
        if not isinstance(data, dict):
    ...
    def is_drm_ready(self) -> Optional[bool]:
        ...
        try:
            ...
            if isinstance(data, bool):
                return data
            elif isinstance(data, dict) and "ready" in data and isinstance(
                data["ready"], bool
            ):
                return data["ready"]
        except BackendError as e:
...
class PrimeAPI:
    ...
    def __init__(
        self, backend_id: Optional[str] = None, addon: Optional[xbmcaddon.Addon] = None
    ) -> None:
        ...
        try:
            ...
            _log(
                xbmc.LOGINFO,
                f"PrimeHub using direct import backend API: {self.backend_id}",
            )
        except BackendUnavailable:
            _log(
                xbmc.LOGWARNING,
                f"Direct import failed for {self.backend_id}. Trying JSON-RPC fallback.",
            )
            try:
                ...
                _log(
                    xbmc.LOGINFO,
                    f"PrimeHub using JSON-RPC backend API: {self.backend_id}",
                )
            except BackendUnavailable:
                _log(xbmc.LOGERROR, "All backend integration strategies failed.")
                raise BackendUnavailable(
                    "Could not connect to backend addon via direct import or JSON-RPC."
                )
...
def get_backend(backend_id: Optional[str] = None) -> PrimeAPI:
    ...
    if _backend_instance is None or (
        backend_id and _backend_instance.backend_id != backend_id
    ):
